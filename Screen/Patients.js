...
import { getAllPatientDonorProfile } from "../Actions/donorActions";
    ...
    <View>
      ...
      >
        ...
        <Text style={styles.headTxt2}>
          Patients Are Looking For Your Help! Help Them
        </Text>
      </ImageBackground>
      <ScrollView
        vertical={true}
      ...
      >
        <View style={{ padding: 14 }}>
          {donor.patient ? (
            donor.patient.map((ptnt, i) => (
              <View key={i} style={styles.listCard}>
                <Text
                  style={{ fontSize: 10, color: "gray", marginLeft: "auto" }}
                >
                  Posted at {moment(ptnt.createdAt).fromNow(true)} ago
                </Text>
                <Text
                  style={{
                    fontWeight: "bold",
                ...
                <View style={{ display: "flex", flexDirection: "row" }}>
                  <Text>{ptnt.age} Years</Text>
                  <Text style={{ marginLeft: 10 }}>
                    Blood Group: {ptnt.bloodgrp.name}
                  </Text>
                </View>
                <Text>{ptnt.gender}</Text>
                ...
                <TouchableOpacity>
                  <Text
                    onPress={() => Linking.openURL(`tel:${ptnt.contactnumber}`)}
                    style={styles.cntBtn}
                  >
                    Contact Urgently
                  </Text>
